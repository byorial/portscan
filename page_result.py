...
class PageScanResult(PluginPageBase):

    def __init__(self, P, parent):
        super(PageScanResult, self).__init__(P, parent, name='result')

    def process_menu(self, req):
        arg = P.ModelSetting.to_dict()
        logger.error(f'[scan_result] -----------------req({req}, {req.args})')
    ...
    def process_command(self, command, arg1, arg2, arg3, req):
        ret = {'ret':'success'}
        data = None
        logger.info(f'[page_result] process_command: {command}, {arg1}, {req}')
        if command == 'web_list':
